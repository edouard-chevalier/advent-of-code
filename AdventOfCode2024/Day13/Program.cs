...
const string inputReal = @"Button A: X+27, Y+65
Button B: X+58, Y+17
Prize: X=2211, Y=4587
...
Button B: X+11, Y+47
Prize: X=1676, Y=4062";
const string inputTest = @"Button A: X+94, Y+34
Button B: X+22, Y+67
Prize: X=8400, Y=5400
...
var regexA = new Regex(@"Button A: X\+(?<x>\d+), Y\+(?<y>\d+)");
var regexB = new Regex(@"Button B: X\+(?<x>\d+), Y\+(?<y>\d+)");
var regexPrize = new Regex(@"Prize: X=(?<x>\d+), Y=(?<y>\d+)");

void Part1(string input) {
    var lines = input.Split(Environment.NewLine);
    var buttonsA = new List<(int x, int y)>();
    var buttonsB = new List<(int x, int y)>();
    var prizes = new List<(int x, int y)>();
    ...
    foreach (var line in lines) {
        ...
        if (matchA.Success) {
            buttonsA.Add((int.Parse(matchA.Groups["x"].Value), int.Parse(matchA.Groups["y"].Value)));
        }
        
        if (matchB.Success) {
            buttonsB.Add((int.Parse(matchB.Groups["x"].Value), int.Parse(matchB.Groups["y"].Value)));
        }
        
        if (matchPrize.Success) {
            prizes.Add((int.Parse(matchPrize.Groups["x"].Value), int.Parse(matchPrize.Groups["y"].Value)));
        }
    }

    long scoreSum = 0;
    for (int machine = 0; machine < prizes.Count; machine++) {
        var prize = prizes[machine];
        int score = int.MaxValue;
        int minA = -1;
        int minB = -1;
        for (int i = 1; i <= 100; i++) {
            for( int j = 1; j <= 100; j++) {
                if( buttonsA[machine].x * i + buttonsB[machine].x * j == prize.x && buttonsA[machine].y * i + buttonsB[machine].y * j == prize.y) {
                    var current = 3 * i + j;
                    if( current < score) {
                        score = current;
                        minA = i;
        ...
        Console.WriteLine($"Prize: X={prize.x}, Y={prize.y} => #A {minA} #B { minB}  {score}");

...
const string inputReal = @"..............423....688..934............970................................95.728..........896...113..................153..972.............
...122..................*.....*..........................919..509*..........&...@.........../...........................+.......*...........
...
const string inputTest = @"467..114..
...*......
..35..633.
......#...
617*......
...
char[][] tab = input.Split(Environment.NewLine).Select(x => x.ToCharArray()).ToArray();

bool IsInTab(int i, int j) {
    return i >= 0 && i < tab.Length && j >= 0 && j < tab[i].Length;
}

bool IsSymbol(int i, int j) {
    return  IsInTab(i,j) && !char.IsDigit(tab[i][j] ) && tab[i][j] != '.';
}
bool IsAdjacentToSymbol(int i, int j)
{
    return IsInTab(i, j) &&
           (IsSymbol(i - 1, j) || IsSymbol(i + 1, j) || IsSymbol(i, j - 1) || IsSymbol(i, j + 1) ||
             IsSymbol(i-1,j-1) || IsSymbol(i-1,j+1) || IsSymbol(i+1,j-1) || IsSymbol(i+1,j+1)
           );
}

bool IsDigit(int i, int j) {
    return IsInTab(i,j) && char.IsDigit(tab[i][j] );
}

( int PartNumber , int NbCol) GetPartNumber(int i, int j) {
    if( !IsDigit(i,j) ) return (0,0);
    int res = tab[i][j] - '0';
    (int a, int b) = GetPartNumber(i, j + 1);
    if (b == 0) {
        return ( res ,1 );
    }

    int pow = (int)Math.Pow(10,b);
...
for( int i = 0; i < tab.Length; i++) {
    for( int j = 0; j < tab[i].Length; j++) {
        if( IsDigit(i,j) ) {
            (int a, int b) = GetPartNumber(i,j);
            bool adj = false;
            for (int k = 0; k < b; k++) {
                if( IsAdjacentToSymbol(i,j+k) )
                {
                    adj = true;
                }
            }
            var s = adj ? " ( adjacent )" : "";
            Console.WriteLine($"({i},{j}) = {a} {b}" + s);
...
bool IsGear(int i, int j) {
    return  IsSymbol(i, j) &&  tab[i][j] == '*';
}
bool IsAdjacentToSymbol(int i, int j)
{
    return IsInTab(i, j) &&
...
List<(int i,int j)> AdjacentGears(int i, int j) {
    if (!IsDigit(i, j)) {
...
for( int i = 0; i < tab.Length; i++) {
    for( int j = 0; j < tab[i].Length; j++) {
        if( IsDigit(i,j) ) {
            (int a, int b) = GetPartNumber(i,j);
            HashSet<(int i,int j)> adjGears = new();
            for (int k = 0; k < b; k++) {
                if( IsAdjacentToSymbol(i,j+k) )
                {
                    adjGears.UnionWith(AdjacentGears(i,j+k));
                }
            }
            var s = adjGears.Count > 0 ? $" ( adjacent gears :{adjGears.Count} )" : "";
            Console.WriteLine($"({i},{j}) = {a} {b}" + s);
            foreach (var adjGear in adjGears)
            {
                if( !gears.TryGetValue(adjGear, out var l) )
                {
                    l = new List<int>();
                    gears.Add(adjGear, l);
                } 
                l.Add(a);
                Console.WriteLine($"  {adjGear} = {gears}");
...
long sum = 0;
foreach (var tuple in gears.Where( x => x.Value.Count == 2))
{
    Console.WriteLine($"{tuple.Key} = {string.Join(",",tuple.Value)} ( {tuple.Value[0] * tuple.Value[1]} )");
sum+= tuple.Value[0] * tuple.Value[1];
}
Console.WriteLine(sum);

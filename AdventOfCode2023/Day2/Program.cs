...
const string inputReal = @"Game 1: 2 red, 2 green; 6 red, 3 green; 2 red, 1 green, 2 blue; 1 red
...
const string inputTest = @"Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
( string Color, int nb) ColorSet(string s) {
    if( s.Contains("red")) return ("red", int.Parse(s.Replace("red", "").Trim()));
    if( s.Contains("blue")) return ("blue", int.Parse(s.Replace("blue", "").Trim()));
    if( s.Contains("green")) return ("green", int.Parse(s.Replace("green", "").Trim()));
...
{
    var s = game.Split(":");
    var gameName = int.Parse(s[0].Replace("Game", "").Trim());
    var rounds = s[1].Split(";", StringSplitOptions.RemoveEmptyEntries);
    bool allRoundPossible = true;
    foreach (var round in rounds)
    {
        var setForRound = round.Split(",", StringSplitOptions.RemoveEmptyEntries);
        if (setForRound.Length > 3) throw new ArgumentException();
            
            var colorSets = setForRound.Select(ColorSet)
            .ToDictionary(x => x.Color, x => x.nb);
        if ((colorSets.TryGetValue("red", out var nbRed) && nbRed > 12) ||
            (colorSets.TryGetValue("blue", out var nbBlue) && nbBlue > 14) ||
            (colorSets.TryGetValue("green", out var nbGreen) && nbGreen > 13))
        {
            allRoundPossible = false;
        }
    }
    if(allRoundPossible) sum += gameName;
}
...
foreach (var game in games)
{
    ...
    bool allRoundPossible = true;
    foreach (var round in rounds) {
        var colorSets = round.Split(",", StringSplitOptions.RemoveEmptyEntries).Select(ColorSet)
            .ToDictionary(x => x.Color, x => x.nb);
        if (colorSets.TryGetValue("red", out var nbRed))
        {
            maxRed = Math.Max(maxRed, nbRed);
        }

        if (colorSets.TryGetValue("blue", out var nbBlue))
        {
            maxBlue = Math.Max(maxBlue, nbBlue);
        }

        if (colorSets.TryGetValue("green", out var nbGreen))
        {
            maxGreen = Math.Max(maxGreen, nbGreen);
        }
    }

    long power = maxRed * maxBlue * maxGreen;

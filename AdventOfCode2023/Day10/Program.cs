...
const string inputReal = @"F7F.7-7FF77|7.|-7-7-.|7F-7-|7-.|-FJ-.L7-J.F7..F---F-7-FF777.L7.FJ---.FFF|--7F-JF--.|J-|F-.LL7FFF7-JFJ--F-7L|7.J-L7FF-F7F|7-FFL7-L.--J77--F|7
...
const string inputTest = @"7-F7-
.FJ|7
SJLL7
|F--J
LJ.LJ";
const string inputTest2 = @"FF7FSF7F7F7F7F7F---7
L|LJ||||||||||||F--J
FL-7LJLJ||||||LJL-77
F--JF--7||LJLJ7F7FJ-
L---JF-JLJ.||-FJLJJ7
...
L.L7LFJ|||||FJL7||LJ
L7JLJL-JLJLJL--JLJ.L";
const string inputTest3 = @".F----7F7F7F7F-7....
.|F--7||||||||FJ....
.||.FJ||||||||L7....
FJL7L7LJLJ||LJ.L-7..
L--J.L7...LJS7F-7L7.
...
// var scores = Enumerable.Range(0, grid.Length).Select(_ => new int[grid[0].Length]).ToArray();
// var visited = Enumerable.Range(0, grid.Length).Select(_ => new bool[grid[0].Length]).ToArray();
...
// }
// bool HasUp(int i, int j) {
//     return HasCell(i, j) && grid[i][j] switch {
//         '|' => true,
//         'J' => true,
//         'L' => true,
//         'S' => true,
...
//     };
// }
// bool GoUp(ref int i, ref int j) {
//     if ( !HasVisited(i-1,j) && HasUp(i,j) && HasDown(i - 1, j) ) {
//         i--;
//         return true;
//     }
//
...
// }
//
// bool GoDown(ref int i, ref int j) {
//     if ( HasDown(i,j) && !HasVisited(i+1,j) && HasUp(i + 1, j) ) {
//         i++;
//         return true;
//     }
//
...
//
//
// bool GoLeft(ref int i, ref int j) {
//     if ( HasLeft(i,j) && !HasVisited(i,j-1) && HasRight(i , j-1) ) {
//         j--;
//         return true;
//     }
//
//     return false;
// }
// bool GoRight(ref int i, ref int j) {
//     if ( HasRight(i,j) && !HasVisited(i,j+1) && HasLeft(i , j+1) ) {
//         j++;
//         return true;
//     }
//
...
// {
//     int currenti = i;int currentj = j;
//     int currentScore = 1;
//     do {
//         visited[currenti][currentj] = true;
//         if (GoRight(ref currenti, ref currentj) || GoDown(ref currenti, ref currentj) ||
//             GoLeft(ref currenti, ref currentj) || GoUp(ref currenti, ref currentj)) {
//             scores[currenti][currentj] = ++currentScore;
//         }
//     }while( !IsStart(currenti, currentj) );
// }
// Map(si,sj+1);
//
//
// Console.WriteLine((scores[si][sj]+1)/2);
// for (int i = 0; i < grid.Length; i++)
// {
//     var sb = new StringBuilder();
//     for (int j = 0; j < grid[i].Length; j++)
//     {
//         sb.Append(scores[i][j] == 0 ? ' ' : grid[i][j]);
//     }
...
var scores = Enumerable.Range(0, grid.Length).Select(_ => new int[grid[0].Length]).ToArray();
var visited = Enumerable.Range(0, grid.Length).Select(_ => new bool[grid[0].Length]).ToArray();
var inside = Enumerable.Range(0, grid.Length).Select(_ => new bool[grid[0].Length]).ToArray();
bool HasVisited(int i, int j) {
    return visited[i][j];
}
bool HasCell(int i, int j) {
...
void MarkOutside(int i, int j)
{
    if (!HasCell(i, j) || visited[i][j]) {
        return;
    }
    grid[i][j] = 'O';
}
for (int step = 0; step < directions.Count; step++)
{
    var (i,j) = path[step];
    var c = grid[i][j];
    if (directions[step] == Direction.Right)
    {
        if (c == 'F')
        {
            MarkOutside(i-1,j);
    ...
    if (directions[step] == Direction.Left)
    {
        if (c == '7')
        {
           // MarkOutside(i-1,j);
    ...
    if (directions[step] == Direction.Up)
    {
        if (c == 'L')
        {
             MarkOutside(i+1,j);
    ...
    if (directions[step] == Direction.Down)
    {
        if (c == '7')
        {
            MarkOutside(i-1,j);
...
var parents = new Dictionary<(int i, int j), (int i, int j)>();
(int i, int j) Find( (int i , int j) cell) {
    if (parents[cell] == cell) {
        return cell;
    }

    return parents[cell] = Find(parents[cell]);
}
void Union( (int i , int j) cell1, (int i , int j) cell2) {
    var p1 = Find(cell1);
    var p2 = Find(cell2);
    if (p1 == p2) {
        return;
...
for (int i = 0; i < grid.Length; i++) {
    for (int j = 0; j < grid[i].Length; j++) {
        ...
        parents[(i,j)] = (i,j);
        if( HasCell(i-1,j) && parents.ContainsKey((i-1,j)) ) {
            Union((i,j), (i-1,j));
        }
        if (HasCell(i+1,j) && parents.ContainsKey((i+1,j))) {
            Union((i,j), (i+1,j));
        }
        if (HasCell(i,j-1) && parents.ContainsKey((i,j-1))) {
            Union((i,j), (i,j-1));
        }
        if (HasCell(i,j+1) && parents.ContainsKey((i,j+1))) {
            Union((i,j), (i,j+1));
...
/*
bool IsAboveInside(int i, int j, HashSet<(int i , int j)> cellsOfGroup) {
    if (!HasCell(i - 1, j)) {
        return false;
    }
    if( cellsOfGroup.Contains((i-1,j)) ) {
        return true;
    }
    var c = grid[i - 1][j];
    if( inside[i-1][j] ) {
...
bool IsBelowInside(int i, int j, HashSet<(int i , int j)> cellsOfGroup) {
    if (!HasCell(i + 1, j)) {
        return false;
    }
    if( cellsOfGroup.Contains((i+1,j)) ) {
...
bool IsRightInside(int i, int j, HashSet<(int i , int j)> cellsOfGroup) {
    if (!HasCell(i , j+1)) {
        return false;
    }
    if( cellsOfGroup.Contains((i,j+1)) ) {
        return true;
    }
...
bool IsLeftInside(int i, int j, HashSet<(int i , int j)> cellsOfGroup) {
    if (!HasCell(i , j-1)) {
        return false;
    }
    if( cellsOfGroup.Contains((i,j-1)) ) {
...
bool IsInside(int i, int j, HashSet<(int i , int j)> cellsOfGroup)
{
    return IsRightInside(i, j, cellsOfGroup) && IsLeftInside(i, j, cellsOfGroup) && IsAboveInside(i, j, cellsOfGroup) &&
           IsBelowInside(i, j, cellsOfGroup);
}*/
//
// var groups = parents.ToLookup(kv => kv.Value, kv => kv.Key);
// var insideGroups = new HashSet<(int i, int j)>();
// int sum = 0;
// foreach (var group in groups)
// {
//     var insideGroup = true;
//     var cellsOfGroup = group.ToHashSet();
//     foreach (var cell in group) {
//         if (!IsInside(cell.i,cell.j,cellsOfGroup)) {
//             insideGroup = false;//break on 5/11
//             break;
//         }
//     }
//
//     if (insideGroup)
//     {
//         insideGroups.Add(group.Key);
//         sum += group.Count();
//     }
// }
var groups = parents.ToLookup(kv => kv.Value, kv => kv.Key);
 var outsideGroups = new HashSet<(int i, int j)>();
 foreach (var group in groups)
 {
        var outsideGroup = group.Any( c=> grid[c.i][c.j] == 'O');
...
for (int i = 0; i < grid.Length; i++)
{
    ...
    for (int j = 0; j < grid[i].Length; j++)
    {
        ...
        // {
        //     c = ' ';
        //     if (parents.ContainsKey((i, j)))
        //     {
        //         if( insideGroups.Contains(Find((i,j))) ) {
        //             c = 'I';
        //         }
        //         else
        //         {

...
const string inputTest = @"two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
...
const string inputTest2 = @"two1nine
sevenine";

const string inputReal = @"zlmlk1
...
foreach( var line in input.Split(Environment.NewLine)) {
    ...
    int val = int.Parse(numbers.First().Value) * 10 + int.Parse(numbers.Last().Value);
...
int toValue(string someVal)
{
    if( someVal == "one") return 1;
    if( someVal == "two") return 2;
    if( someVal == "three") return 3;
    if( someVal == "four") return 4;
    if( someVal == "five") return 5;
    if( someVal == "six") return 6;
    if( someVal == "seven") return 7;
    if( someVal == "eight") return 8;
    if( someVal == "nine") return 9;
    return int.Parse(someVal);
}
foreach( var line in input.Split(Environment.NewLine))
{
    ...
    for( int i = 0;i< line.Length;i++) {
        ...
        var subLine = line.Substring(i);
            var number = Regex.Matches(subLine, @"^(one|two|three|four|five|six|seven|eight|nine)");
            if( number.Count == 1) { 
                numbers.Add(number.First().Value);
            }
    ...
    int val = toValue(numbers.First()) * 10 + toValue(numbers.Last());

...
const string inputReal = @"mhx: tjz
tgh: vdj xzg dmm zrk
sjv: tjz gbf qlc
ltb: hnh flx ktb
gfh: nkk njc
...
nlp: ssb dzh gbl
hzb: dpc fsg plx qjt";
const string inputTest = @"jqt: rhn xhk nvd
rsh: frs pzl lsr
xhk: hfx
cmg: qnr nvd lhk bvb
rhn: xhk bvb hfx
...
void Part1(string input) {
    ...
    void AddEdge(string from, string to) {
        if (!edges.ContainsKey(from)) {
            edges[from] = new HashSet<string>();
        }
        edges[from].Add(to);
        if (!edges.ContainsKey(to)) {
            edges[to] = new HashSet<string>();
        }
        edges[to].Add(from);
    }
    ...
    Console.WriteLine($"Nodes: {edges.Count}");
    Console.WriteLine($"Edges: {edges.Sum(e => e.Value.Count)}");
    var rnd = new Random(45);

    bool Karger()
    {
        var graph = edges.ToDictionary(kv => kv.Key, kv => kv.Value.ToDictionary(s=>s,s=>1));
        var nodesCount = graph.Keys.ToDictionary(s=>s, s=>1);
        void AddEdgeWithCount(string from, string to, int count) {
        ...
        while (graph.Count > 2) {
            var edge = graph.ElementAt(rnd.Next(graph.Count));
            var node1 = edge.Key;
            var node2 = edge.Value.ElementAt(rnd.Next(edge.Value.Count));
            if (node1 == node2.Key) {
                continue;
            }
            var node2Edges = graph[node2.Key];
            foreach (var node in node2Edges) {
                if (node.Key == node1) {
                    continue;
                }
                AddEdgeWithCount(node1, node.Key, node.Value);
                graph[node.Key].Remove(node2.Key);
            }

            graph[node1].Remove(node2.Key);
            graph.Remove(node2.Key);
            nodesCount[node1] += nodesCount[node2.Key];
            nodesCount.Remove(node2.Key);
        }
        var first = graph.First();
        var second = graph.Last();
        var minCut = first.Value.Sum(e => e.Value);
        if (minCut == 3)
        {
            Console.WriteLine(nodesCount[first.Key]*nodesCount[second.Key]);
        }
        return minCut == 3;
    }
